...
def _apply_theme(app: Any, theme: str) -> None:
    """Apply Bootstrap theme to app."""
    if not hasattr(app, '__call__'):
        return
    
    # Wrap app to inject theme
    original_call = app.__call__
    
    def themed_call(*args: Any, **kwargs: Any) -> Any:
        result = original_call(*args, **kwargs)
        
        # Inject theme into HTML if it's a string response
        if isinstance(result, str) and '<html' in result:
            result = result.replace('<html', f'<html data-bs-theme="{theme}"', 1)
        
        return result
    
    app.__call__ = themed_call